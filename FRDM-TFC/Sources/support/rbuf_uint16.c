...
void
rbuf_uint16_init(rbuf_uint16_s *rbuf, uint16_t* buf, uint32_t size) {
    rbuf->size = size;
    rbuf->buf = buf;
...
static void
rbuf_uint16_update_size(rbuf_uint16_s *rb) {
    if(rb->wfx == rb->rfx)
        rb->used = 0;
...
void
rbuf_uint16_set_mode(rbuf_uint16_s *rbuf, rbuf_uint16_mode_t mode)
{
    rbuf->mode = mode;
}

rbuf_uint16_mode_t
rbuf_uint16_mode(rbuf_uint16_s *rbuf)
{
    return rbuf->mode;
}

void
rbuf_uint16_skip(rbuf_uint16_s *rb, uint32_t size) {
    ...
    rbuf_uint16_update_size(rb);
}

uint32_t
rbuf_uint16_read(rbuf_uint16_s *rb, uint16_t *out, uint32_t size) {
...
uint32_t
rbuf_uint16_write(rbuf_uint16_s *rb, uint16_t *in, uint32_t size) {
...
uint32_t
rbuf_uint16_used(rbuf_uint16_s *rb) {
    return rb->used;
}

uint32_t
rbuf_uint16_size(rbuf_uint16_s *rb) {
    return rb->size - 1;
}

uint32_t
rbuf_uint16_available(rbuf_uint16_s *rb) {
    return rb->available;
}

void
rbuf_uint16_clear(rbuf_uint16_s *rb) {
    rb->rfx = rb->wfx = 0;
    rbuf_uint16_update_size(rb);
}

uint32_t
rbuf_uint16_find(rbuf_uint16_s *rb, uint16_t octet) {
    uint32_t i;
    uint32_t to_read = rbuf_uint16_used(rb);
    if (to_read == 0)
        return -1;
...
uint32_t
rbuf_uint16_read_until(rbuf_uint16_s *rb, uint16_t octet, uint16_t *out, uint32_t maxsize)
{
    uint32_t i;
    uint32_t size = rbuf_uint16_used(rb);
    ...
    rbuf_uint16_skip(rb, (size - to_read));
    return (size-to_read);
}

static uint32_t
rbuf_uint16_copy_uint32_ternal(rbuf_uint16_s *src, rbuf_uint16_s *dst, uint32_t len, uint32_t move)
{
    if (!src || !dst || !len)
        return 0;

    uint32_t to_copy = rbuf_uint16_available(dst);
    if (len < to_copy)
        to_copy = len;

    uint32_t available = rbuf_uint16_used(src);
    ...
    if (contiguous >= to_copy) {
        if (move) {
            rbuf_uint16_read(src, &dst->buf[dst->wfx], to_copy);
        } else {
            if (src->rfx < src->wfx) {
        ...
        if (move) {
            rbuf_uint16_read(src, &dst->buf[dst->wfx], contiguous);
            rbuf_uint16_read(src, &dst->buf[0], remainder);
        } else {
            if (src->rfx < src->wfx) {
    ...
    rbuf_uint16_update_size(dst);
    return to_copy;
}

uint32_t
rbuf_uint16_move(rbuf_uint16_s *src, rbuf_uint16_s *dst, uint32_t len)
{
    return rbuf_uint16_copy_uint32_ternal(src, dst, len, 1);
}

uint32_t
rbuf_uint16_copy(rbuf_uint16_s *src, rbuf_uint16_s *dst, uint32_t len)
{
    return rbuf_uint16_copy_uint32_ternal(src, dst, len, 0);
}
