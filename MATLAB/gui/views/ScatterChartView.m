classdef ScatterChartView < AxisView
    %ScatterChartView
    ...
    methods
        % ScatterChartView constructor
        function obj = ScatterChartView(session, attribute, varargin)
            % parse input
            p = inputParser;
    ...
    methods
        ...
        function obj = draw(obj, x, y)
            ...
            if (isempty(obj.hplot)) % initialise plot
                obj.hplot = scatter(obj.haxis, x, y); % returns one handle for each line (i.e. if x is 2-dimensional)
                obj = label(obj);
